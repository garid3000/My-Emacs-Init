...
;;(global-set-key (kbd "M-w") 'ace-window)           ;; do I need this?
;;(setq aw-keys '(?a ?s ?d ?f ?g ?h ?j ?k ?l))       ;; do I need this?
#+end_src
** new general config
*** general
#+begin_src emacs-lisp
...
| n      |        | dired-sidebar-jump-to-sidebar                           | (n)-side-tree           |
| N      |        | dired-sidebar-hide-sidebar                              | (N)-side-tree-close     |
| ZQ     |        | kill-emacs                                              | quit                    |
| l      |        | consult-buffer                                          | (l)ist-buffers          |
| Lr     |        | revert-buffer-quick                                     | (L-r)evert              |
| Le     |        | eval-buffer                                             | (L-e)val                |
| Lb     |        | ibuffer                                                 | (L-(b)uffer             |
| Li     |        | ibuffer                                                 | (L-i)buffer             |
| Ln     |        | rename-buffer                                           | (L-n)ame                |
| Lp     |        | switch-to-prev-buffer                                   | (L-p)rev                |
| LP     |        | switch-to-next-buffer                                   | (L-P)next               |
| Ld     |        | crux-delete-file-and-buffer                             | (L-d)delete             |
| ca     |        | calc                                                    | (ca)lc                  |
| mg     |        | magit-status                                            | (m)a(g)it               |
| co     |        | consult-org-heading                                     | (c-o)rg                 |
| cm     |        | consult-imenu                                           | (c-i(m)enu              |
...
| bm     |        | consult-bookmark                                        | (b)ook(m)ark-consult    |
| bs     |        | bookmark-set                                            | (b)m-(s)et              |
| bd     |        | bookmark-delete                                         | (b)m-(d)elete           |
| br     |        | bookmark-rename                                         | (b)m-(r)ename           |
| pf     |        | project-find-file                                       | (p-f)ile                |
| pd     |        | consult-flymake                                         | (p-d)iagnostic          |
| pr     |        | xref-find-references                                    | (p-r)eferences          |
| pz     |        | project-switch-project                                  | (p-z)                   |
| ps     |        | project-find-regexp                                     | (p-s)find-regexp        |
| vq     |        | format-all-buffer                                       | (q)-format-buffer       |
| vr     |        | project-query-replace-regexp                            | (p-r)eplace             |
| pk     |        | project-kill-buffers                                    | (p-k)ill-buffers        |
| pl     |        | consult-project-buffer                                  | (p-l)ist-buffers        |
| pL     |        | project-list-buffers                                    | (p-L)ist-buffers        |
| e      |        | eshell                                                  | (e)shell                |
| vt     |        | vterm                                                   | (v)ir-(t)erm            |
| oe     | y      | (find-file "~/.config/emacs/init.org")                  | open emacs conf org     |
| ob     | y      | (find-file (nth 0 org-cite-global-bibliography))        | open bibliography       |
| oE     | y      | (find-file "~/.config/emacs/init.el")                   | open emacs conf el      |
| oC     | y      | (find-file org-default-notes-file)                      | open Capture file       |
| <      |        | recompile                                               | recompile               |
| ruim   |        | org-roam-ui-mode                                        | (r-ui)-(m)ode           |
| Dr     |        | denote-rename-file                                      | (D)enote-(D)ired        |
| Dn     |        | denote-open-or-create                                   | (D)e-(N)ote             |
| DD     | y      | (dired denote-directory)                                | (D)enote-(D)ired        |
| h      |        | describe-symbol                                         | Describe-Sym            |
| H      |        | describe-key                                            | Describe-Key            |
| sl4    | y      | (setq display-line-numbers-width 4)                     | (s)ys-number(l)ine-(4)  |
| sl3    | y      | (setq display-line-numbers-width 3)                     | (s)ys-number(l)ine-(3)  |
| srf    | y      | (recentf-open-files)                                    | (s-r)ecent-(f)iles      |
| st     |        | gry/open-term-at                                        | (s-t)erminal            |
| sT     |        | toggle-transparency                                     | (s-T)oggle-transparency |
| sns    |        | crux-create-scratch-buffer                              | (s-n)ew-(s)cratch-buf   |
| slm    |        | scroll-lock-mode                                        | (s)croll-(l)ock-(m)ode  |
| scl    | y      | (global-command-log-mode) (clm/open-command-log-buffer) | (s-c)md-(l)og           |
| svl    |        | global-display-fill-column-indicator-mode               | (s-v)ertical-(l)ine     |
| sw     |        | whitespace-mode                                         | (s-w)hitespace          |
| sco    |        | corfu-mode                                              | (s-c)orfu-(m)ode        |
| se     |        | eglot                                                   | (s-e)glot               |
| sib    |        | clone-indirect-buffer                                   | (s-i)ndirect-(b)uffer   |
| rg     |        | rg                                                      | (r)ip-(g)rep            |
| yn     |        | yas-new-snippet                                         | (y)as-(n)ew             |
| yv     |        | yas-visit-snippet-file                                  | (y)as-(v)isit           |
| yd     |        | yas-describe-tables                                     | (y)as-(d)esc-table      |
| yc     |        | consult-yasnippet                                       | (y)as-(c)onsult         |
| ce     |        | calc-embedded                                           | (c)alc-(e)mbedded       |
| cF     |        | full-calc                                               | (c)alc-(F)ull           |
| za     |        | garid-custom/align-regexp-and-untabify                  | (z)-reg-(a)lign         |
| zr     |        | replace-regexp                                          | (z)-reg-(r)eplace       |
| zh     |        | highlight-symbol-at-point                               | (z)-(h)ighlight         |
| zH     |        | unhighlight-regexp                                      | (z)-(H)ighlight-off     |
| oTm    |        | orgtbl-mode                                             | (o-T)able-(m)ode        |
| oTc    |        | orgtbl-create-or-convert-from-region                    | (o-T)able-(c)reate      |
