...
(use-package evil-visual-mark-mode
  ...
  :after evil
  :general
  (:states 'normal "<f3>" 'evil-visual-mark-mode)
  )
#+end_src
...
** TODO =dired= - Directory [0/1]

- [ ] Need to bind SPC to nil in the code
...
** TODO =dired-filter= need to do further the key-bind [0/1]

- [ ] mostly because of the SPC is still bind to dired-next in the dired-mode-map
...
(use-package dired-filter
  ...
  (:states 'normal
           :keymaps   'dired-mode-map
           "SPC f"  'dired-filter-by-name
           "SPC F"  'dired-filter-pop
           ))
#+end_src
...
(use-package recentf
  ...
  :hook
  (after-init . recentf-mode) ;; :config (recentf-mode 1)
  )
#+end_src
** =replace=
#+begin_src emacs-lisp
...
(use-package org
  ...
  (org-tag-alist           '((:startgroup . nil)
                             ...
                             ("uavsoft"   . ?U)
                             ("ernd"      . ?E)))
  (org-return-follows-link      t) ;; org-keys? Package?
...
(use-package ob-mermaid
  ...
  :straight t
  :custom
  (ob-mermaid-cli-path "mmdc") ;; or  "/usr/bin/mmdc"
  )
#+end_src
...
(use-package denote
  ...
  ;; can be list  ;; dynamically getting the list via name if it starts with "denote_"
  (denote-directory (mapcar
...
- [ ]  =:demand t= was required because =:map org-gtd-clarify-mode-map ("C-c C-c" . org-gtd-organize)= is not working
...
** =edraw= SVG drawing inline in org                                                    :here:
#+begin_src emacs-lisp
...
(use-package ol-notmuch
  :straight  t
  ;;:config
  ;;(define-key global-map "\C-c l" 'org-store-link) ;; make is more :general later
