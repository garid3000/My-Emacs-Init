...
(use-package repeat
  :straight nil
  :hook 
  (emacs-startup-hook . repeat-mode) ;; (after-init-hook . repeat-mode)
  )
#+end_src
...
(use-package evil
  ...
  :bind (:repeat-map evil/window-related
...
(use-package evil-collection
  ...
  ;;  '(
  ;;    org org-present outline
  ;;    vundo bookmark
  ;;    (magit magit-submodule) magit-repos magit-section  magit-todos
  ;;    gptel
  ;;    info
  ;;    notmuch
  ;;    (pdf pdf-view)
  ;;    ;; dired dired-sidebar wdired
  ;;    diff-hl diff-mode ediff
  ;;    ))

  ;; maybe use;
  ;; bm calc calendar comint compile consult corfu (csv "csv-mode")
  ;;  eglot flycheck flymake  grep
  ;; ibufferr  pass  python rg ripgrep
  ;; smerge-mode trashed vertico which-key xref

  ;; stoped
  ;; dired dired-sidebar wdired

  ;; bluetooth ag alchemist anaconda-mode apropos arc-mode
  ;; atomic-chrome 2048-game kotlin-mode eat auto-package-update
  ;; beginend (buff-menu "buff-menu") bufler cider citre cmake-mode
  ;; color-rg company crdt (custom cus-edit) cus-theme dape dashboard
  ;; daemons deadgrep debbugs debug devdocs dictionary difftastic
  ;; disk-usage distel doc-view docker ebib ebuku edbi edebug elpaca
  ;; ement explain-pause-mode eldoc elfeed elisp-mode elisp-refs
  ;; elisp-slime-nav embark emms emoji epa ert eshell eval-sexp-fu
  ;; evil-mc eww fanyi finder forge free-keys geiser ggtags
  ;; git-timemachine gited gnus go-mode guix hackernews helm help
  ;; helpful hg-histedit hideshow hungry-delete hyrolo (image
  ;; image-mode) image-dired image+ imenu imenu-list (indent "indent")
  ;; indium  ivy js2-mode kmacro leetcode lispy lms log-edit
  ;; log-view lsp-ui-imenu lua-mode macrostep man markdown-mode
  ;; message minesweeper monky mpc mpdel mpdired mu4e
  ;; mu4e-conversation neotree newsticker nov omnisharp
  ;; org-roam osx-dictionary outline ovpn-mode p4 (package-menu
  ;; package) popup proced (process-menu simple) prodigy profiler
  ;; p-search quickrun racer racket-describe reader realgud reftex
  ;; replace restclient rjsx-mode robe rtags ruby-mode scheme
  ;; scroll-lock selectrum sh-script shortdoc simple simple-mpc slime
  ;; sly snake so-long speedbar tab-bar tablist tabulated-list
  ;; tar-mode telega (term term ansi-term multi-term) tetris thread
  ;; tide timer-list transmission tuareg typescript-mode ultra-scroll
  ;; vc-annotate vc-dir vc-git vdiff view vlf vterm w3m wgrep
  ;; with-editor woman xwidget yaml-mode youtube-dl zmusic (ztree
  ;; ztree-diff ztree-dir)

  ;; :hook
  ;; (after-init-hook . evil-collection-init) ;; 
  :config
  (evil-collection-init)
  ;; I needed to load this as :config, because hooking above
  ;; (evil-collection-init) with after-init-hook causes overrides 
  ;; keybinding setup in the below configurations
  )
#+end_src
...
(use-package evil-textobj-tree-sitter
  ...
  ;; (define-key evil-normal-state-map (kbd "]f") (lambda () (interactive) (evil-textobj-tree-sitter-goto-textobj "function.outer"       ))) ;; Goto start of next function
  ;; (define-key evil-normal-state-map (kbd "[f") (lambda () (interactive) (evil-textobj-tree-sitter-goto-textobj "function.outer" t     ))) ;; Goto start of previous function
  ;; (define-key evil-normal-state-map (kbd "]F") (lambda () (interactive) (evil-textobj-tree-sitter-goto-textobj "function.outer" nil t ))) ;; Goto end of next function
  ;; (define-key evil-normal-state-map (kbd "[F") (lambda () (interactive) (evil-textobj-tree-sitter-goto-textobj "function.outer" t t   ))) ;; Goto end of previous function
...
(use-package evil-visual-mark-mode
  :straight t
  :after evil
  :general (:states 'normal "<f3>" 'evil-visual-mark-mode))
#+end_src
...
(use-package modus-themes
  ...
  ;; (emacs-startup-hook . (lambda () (modus-themes-load-theme (car modus-themes-to-toggle))))
  )
#+end_src
...
(use-package ffap
  ...
  (:states 'normal
           :prefix "SPC"
           "D f"     'ffap           ;; already bind to normal mode: g-f 
           "D F m"   'ffap-menu
           ))
#+end_src
** =files=
...
(use-package imenu
  ...
  (:states 'normal
           :prefix "SPC"
           "i m "     'imenu) ;; technically (originally M-g i)
  )
#+end_src
...
(use-package newcomment
  :straight nil
  :general
  (:states '(normal insert) "C-/"  'comment-line)
  (:states '(visual)        "C-/"  'comment-or-uncomment-region)
  )
#+end_src
...
(use-package dired
  ...
  :custom ((dired-listing-switches  "-agho --group-directories-first")
           ...
           (insert-directory-program (if (eq system-type 'berkeley-unix) "gls"  ;; in case for BSD
                                       "ls")))
  ...
  (:states 'normal
           ...
           (kbd "C-n")  'mkdir
           (kbd "C-s")  'dired-omit-mode
           "SPC D p"    'dired-preview-mode
           "SPC W"      'wdired-change-to-wdired-mode
           ;; dired-filter
           "g f"        'dired-filter-by-name 
           "g F"        'dired-filter-pop 
           )

  (:states 'normal
           :keymaps   'dired-mode-map ;; #'dired-preview-mode-map
           :predicate '(bound-and-true-p dired-preview-mode)
...
(use-package mermaid-mode :after org :straight t)
...
(use-package denote
  :straight t
  :after    org
  :demand   t   ;; because I usually it just after starting emacs 
  :custom
  ...
  (:states  'normal
            ...
            "n R d"  'denote-rename-file-date
            "n R i"  'denote-rename-file-identifier
            "n R s"  'denote-rename-file-signature
            "n R t"  'denote-rename-file-keywords
            "n R k"  'denote-rename-file-keywords
...
(use-package citar-denote
  ...
  (emacs-startup-hook . citar-denote-mode)
  )
#+end_src
...
(use-package org-edna
  ...
  (emacs-startup-hook . org-edna-mode)
  )
#+end_src
*** =org-gtd=
...
(use-package org-gtd
  ...
  (:states 'normal
           :keymaps 'org-mode-map
           :prefix "SPC o"
           "s c"    'org-gtd-clarify-item
           "g c"    'org-gtd-clarify-item
           "g a"    'org-gtd-archive-completed-items
           "g A"    'org-gtd-archive-item-at-point)
  ...
  (:states 'normal
           ...
           "o c  "     'org-gtd-capture)
  ;; :hook
  ;; (emacs-startup-hook . org-gtd-mode) 
  )
#+end_src
...
(use-package edraw-mode
  ...
  ;;   (require 'edraw-org)
  ;;   (edraw-org-setup-default))  ;; this was causing some issues with the org-mode
...
(use-package code-cells
  ...
  :hook (python-base-mode-hook . code-cells-mode)
  :bind (:map code-cells-mode-map
              ...
              ("M-d"        . code-cells-kill           )
              ("M-j"        . code-cells-move-cell-down ) ;; conflict with outline indent mode, maybe later think on it
              ("M-k"        . code-cells-move-cell-up   ) ;; conflict with outline indent mode, maybe later think on it
  ...
  :general
  (:states 'normal
           ;; :keymaps 'python-mode-base-map
           ;; :predicate '(bound-and-true-p code-cells-mode)
...
(use-package prog-mode
  ...
  (:states 'normal
           ...
           "K"      'eldoc
           "g o"     'eglot-find-typeDefinition
           "] d"     'flymake-goto-next-error
           "[ d"     'flymake-goto-prev-error

           "SPC s e" 'eglot
           "SPC v r" 'eglot-rename
           "SPC e h" 'eglot-show-type-hierarchy
           "SPC e h" 'flymake-show-buffer-diagnostics
           "SPC e H" 'flymake-show-project-diagnostics
            ;; "g       d "evil-goto-definition

           "SPC p r" 'xref-find-references
           )
...
(use-package project
  ...
  :general (
            :states 'normal
            :prefix "C-SPC" ;; it does the command in the other window
            ;; "p"      '(:which-key "project")
            ;; "p b"     '(lambda () (interactive) (project-other-window-command) (project-switch-to-buffer (list (project--read-project-buffer))))
            "p c"     '(lambda () (interactive) (project-other-window-command) (project-recompile))
            "p C"     '(lambda () (interactive) (project-other-window-command) (project-compile))
            "p d"     '(lambda () (interactive) (project-other-window-command) (project-dired))
            "p D"     '(lambda () (interactive) (project-other-window-command) (project-find-dir))
            "p e"     '(lambda () (interactive) (project-other-window-command) (project-eshell))
            "p f"     '(lambda () (interactive) (project-other-window-command) (project-find-file))
            "p k"     '(lambda () (interactive) (project-other-window-command) (project-kill-buffers))
            "p l"     '(lambda () (interactive) (project-other-window-command) (consult-project-buffer))
            "p L"     '(lambda () (interactive) (project-other-window-command) (project-list-buffers))
            "p q r r" '(lambda () (interactive) (project-other-window-command) (project-query-replace-regexp))
            "p s"     '(lambda () (interactive) (project-other-window-command) (project-find-regexp))
            "p z"     '(lambda () (interactive) (project-other-window-command) (project-switch-project))
            )
  )
...
(use-package eshell
  ...
  (emacs-startup-hook . eshell-atuin-mode)
  )
#+end_src
...
(use-package notmuch
  ...
  ;; (:states '(normal notmuch-show-mode )
...
(use-package gif-screencast
  ...
  ;; ;; for hyprland
  ;; ;; (setq gif-screencast-program "hyprshot")
  ;; ;; (setq gif-screencast-args    '("-m" "window" "-m" "active" "-o" "/" "-f"))
  )
#+end_src
** garbage-collect
#+begin_src emacs-lisp
...
(use-package google-this
  ...
  (:states '(normal visual)
           :prefix "SPC"
           ;; "G"   '(:which-key "Google...") ;; which stops loading google-translate package
