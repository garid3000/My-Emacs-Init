...
#+begin_src shell :noeval :tangle Makefile
...
;; probably only used first time
;; (unless (package-installed-p 'use-package)
;;   (package-refresh-contents)
;;   (package-install 'use-package))
;;(eval-when-compile (require 'use-package))
;; (use-package auto-package-update) ; Helps with update automoacial
#+end_src
* Vi Movement Related
** vim-undo
#+begin_src emacs-lisp
...
(use-package evil
  ...
  :config
  (evil-define-key 'normal org-mode-map (kbd "TAB") #'org-cycle) ;; cuz https://emacs.stackexchange.com/a/73457/38482
  (evil-mode 1))
#+end_src
** evil-collection
...
#+end_src
** evil-surround
I rarely uses. but it can put quotation mark at the 2 ends of visual region.
...
(use-package org
  ...
  :config
  (setq org-ellipsis                         " ▼" ;; folded subtree symbol
  ...
  ;; if in desktop environment (not from tty terminal)
  ...
  ;; Latex related ===================================================================
  (setq
   org-latex-format-headline-function   'my-org-latex-format-headline-function
...
(with-eval-after-load 'org
  ...
  (add-to-list 'org-structure-template-alist '("she"  .  "src sh"))
...
** org-babel
#+begin_src emacs-lisp
;; may need to load ob-... package before this
;; e.g. may need to load ob-nim before this
...
Defining custom function to toggle visibily of bold, italics, and links etc.
...
* Org Zettelkasten like (roam and denote)
** org roam
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
(use-package denote
  :config (setq denote-directory (expand-file-name "~/Documents/Personal")
...
(use-package org-auto-tangle
  :defer t
  :hook   (org-mode . org-auto-tangle-mode)  ;; <--- I disabled because just to see whether that makes faster or not
  :config
  (setq org-auto-tangle-default t) ;; maybe nil might be good
  )
#+end_src
...
;; org tempo related? TODO need to move to electric pair mode section
;; this is about disabling < parenthesis >
(setq electric-pair-inhibit-predicate ;;; ??forgot what it does?
...
#+name: table_keybindings
| key(s) | function                 | short description |
|--------+--------------------------+-------------------|
