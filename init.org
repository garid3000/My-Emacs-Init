...
* From shell you can run make run:                                                :ATTACH:
:PROPERTIES:
:ID:       20250409T155759.106525
:END:
...
#+begin_src dot :file (my/org-attach-dir "diagram.svg") :eval never-export :tangle no
digraph {
    a[label="init.org"];
    b[label="init.el"];
    a -> b [label="org-babel-tangle"]
}
#+end_src

[[file:data/diagram.svg/diagram.svg]]
...
(use-package evil
  ...
  (define-key evil-normal-state-map (kbd "<f2>") 'evil-avy-goto-char-2)
