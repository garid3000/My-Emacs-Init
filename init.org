...
(use-package org-attach
  ...
  (defun my/org-attach-dir-calculator (id)
    ...
    (and (< 9 (length id))
         ;;(buffer-file-name)
         ;;(format "%s/subtree_id_%s"     <---- i longer want each heading to have separate dir
...
#+begin_src emacs-lisp
(use-package python
  :bind (:repeat-map my/python-repeat-map
...
| =vf=   | =python-nav-forward-defun= | xxxxxxxxxxxxxxx               | -    |        |
...
(garids-keybind/for-all-modes
  ...
  "vf" '(python-nav-forward-defun :which-key "xxxxxxxxxxxxxxx")
