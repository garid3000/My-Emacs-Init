...
| st     |        | garid-custom/open-external-term-here                    | s-(t)erminal externally    |
...
inside-org-file = can be called (only) in org-file (won't be called from different file-type).
...
(garids-keybind/for-all-modes
  ...
  "st" '(garid-custom/open-external-term-here :which-key "s-(t)erminal externally")
