...
#+end_src
** COMMENT =org-num=: org number zero padding
[[https://emacs.stackexchange.com/q/80800/38482][From this answer]]
...
(use-package org-num
  ...
  (setq org-num-format-function
        (lambda (numbering)
          (concat (number-to-string (car numbering))               ;; takes 0-th of seq of numbers
                  "."                                              ;;
                  (mapconcat (lambda (x) (format "%02d" x))        ;; 0 pad
                             (cdr numbering)                       ;; [ 1:... ] of seq of numbers
                             ".")                                  ;; sep
                  " ")))                                           ;; put space
  )
#+end_src
...
(use-package plantuml-mode
  ...
  ;; '(
  ;;   ;; gantts relaeted
  ;;   ("\\<\\(days\\|weeks\\)\\>" .                      font-lock-keyword-face) ;; duration
  ;;   ("\\<\\(starts\\|ends\\|lasts\\)\\>" .             font-lock-keyword-face) ;; verbs - duration
  ;;   ("\\<\\(at\\|after\\|on\\|from\\|to\\|end\\)\\>" . font-lock-keyword-face) ;; prepositions
  ;;   ("\\<\\(then\\)\\>" .                              font-lock-keyword-face) ;; short nickname
  ;;   ("\\<\\(as\\)\\>" .                                font-lock-keyword-face) ;; short nickname
  ;;   ("\\<\\(is\\)\\>" .                                font-lock-keyword-face) ;; is
  ;;   ("\\<\\(completed\\|complete\\)\\>" .              font-lock-keyword-face) ;; completion
  ;;   ("\\<\\(colored\\|in\\)\\>" .                      font-lock-keyword-face) ;; coloring
  ;;   ("\\<\\(happen)\\>" .                              font-lock-keyword-face) ;; verbs - milestone
  ;;   ("\\<\\(links)\\>" .                               font-lock-keyword-face) ;; verbs - milestone
  ;;   ("\\<\\(closed\\|open)\\>" .                       font-lock-keyword-face) ;; closed or open
  ;;   ))
  )
...
(use-package org-roam
  ...
  (org-roam-v2-ack                   t                 )
  (org-roam-directory                "~/BrainDump/roam") ;; (org-roam-directory "~/Pensieve/")
  (org-roam-db-location              "~/BrainDump/roam/0000_database.db" )
  ...
  (org-roam-capture-templates
   `(("d" "Default" plain "* %?"
      :if-new (file+head  "${id}--${slug}.org"
                          ;;"%<%Y%m%dT%H%M%S>--${slug}.org"
                          ;;,(concat (car (split-string "${id}.sdf" "{")) "--" "${slug}.org")
                          ,(concat "#+title: ${title}\n"
                                   "#+filetags: :notag:\n"
                                   "#+date: %U\n"
                                   "#+setupfile: ~/.config/emacs/org-templates/level-0-html-export-template.org\n"
     ...
     ("M" "Math" plain "* %?"
      :if-new (file+head "g_math/%<%Y%m%dT%H%M%S>--${slug}.org"
                         ,(concat "#+title: ${title}\n"
                                  "#+filetags: :math:\n"
                                  "#+author: Garid Zorigoo\n"
                                  "#+date: %U\n"
     ...
     ("g" "Golang Notes" plain "* %?"
      :if-new (file+head "g_go/%<%Y%m%dT%H%M%S>--${slug}.org"
                         ,(concat ;;"#+title: ${title}\n"
                           "#+filetags: :go:\n"
                           ;;"#+author: Garid Zorigoo\n"
                           "#+date: %U\n"
     ...
     ("m" "Meta (meaning access nodes)" plain "* %?"
      :if-new (file+head "%<%Y%m%dT%H%M%S>--${slug}__meta.org"
                         ,(concat "#+title: ${title}\n"
                                  "#+filetags: :meta:\n"
                                  "#+date: %U\n"
                                  "#+setupfile: ~/.config/emacs/org-templates/level-0-html-export-template.org\n"
     ...
     ("P" "Project" plain
      ...
      :if-new (file+head "g_project/%<%Y%m%dT%H%M%S>--${slug}.org"
                         ,(concat "#+title: ${title}\n"
                                  "#+filetags: :Project:\n"
                                  "#+auto_tangle: nil"))
      :unnarrowed t)
...
** Different roam-notes                                                       :customFunc:
#+name: table-roam
| r | =/home/garid/BrainDump/roam_research/= | research |
| o | =/home/garid/BrainDump/roam_old/=      | old      |

#+name: generate-hydra-body-for-choosing-roam-directory
#+header: :var listroam=table-roam
#+begin_src emacs-lisp :results code :wrap src emacs-lisp :tangle no
(append
 `(defhydra garid/hydra-choose-roam-directory ())
 (mapcar
  (lambda (each_roam_dir)
    `(
      ,(nth 0 each_roam_dir)
      (progn
        (setq org-roam-directory    ,(remove-equal-from-both-sides (nth 1 each_roam_dir))
              org-roam-db-location  ,(concat (remove-equal-from-both-sides (nth 1 each_roam_dir)) "0000_database.db"))
...
#+RESULTS: generate-hydra-body-for-choosing-roam-directory
#+begin_src emacs-lisp
(defhydra garid/hydra-choose-roam-directory nil
  ("r"
   (progn
     (setq org-roam-directory "/home/garid/BrainDump/roam_research/" org-roam-db-location "/home/garid/BrainDump/roam_research/0000_database.db")
     (message "Now org-roam-directory = %s" org-roam-directory))
   "research")
  ("o"
   (progn
     (setq org-roam-directory "/home/garid/BrainDump/roam_old/" org-roam-db-location "/home/garid/BrainDump/roam_old/0000_database.db")
     (message "Now org-roam-directory = %s" org-roam-directory))
   "old"))
#+end_src
...
(use-package org-agenda
  ...
  :custom
  (org-agenda-files '(
                      "~/BrainDump/gtd/main.org"
                      ;;"~/BrainDump/denote/phd/20241114T160402--development-of-novel-methodology-for-measuring-hemispherical-direction-reflection-factor-hdrf-in-vegetation-fields-via-utilizing-a-gimbal-motorized-spectrometer__phd_thesis.org"
                      ))
  (org-agenda-span 90)
...
| =rc=   | =garid/hydra-choose-roam-directory/body=                         | roam chande directory                | -    |        |
...
(garids-keybind/for-all-modes
  ...
  "rc" '(garid/hydra-choose-roam-directory/body :which-key "roam chande directory")
...
(custom-set-variables
 ...
 '(safe-local-variable-values
   '((org-roam-db-location . "/home/garid/BrainDump/roam/0000_database.db")
     (org-roam-directory . "/home/garid/BrainDump/roam/")
     (org-roam-db-location . "/home/garid/BrainDump/roam_old/0000_database.db")
     (org-roam-directory . "/home/garid/BrainDump/roam_old/")
