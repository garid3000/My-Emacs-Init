...
(use-package faces
  ...
  (face-font-family-alternatives
   '(("Monospace"        "Iosevka SS06" "Iosevka Term SS06" "Liberation Mono" "courier" "fixed")
     ("Serif"            "Iosevka aile" "Linux Libertine"  "CMU Serif" "Georgia"  "DejaVu Serif" "serif")))
  :custom-face
...
** pulsar of window movement                                                     :by_prot:
#+begin_src emacs-lisp
(use-package pulsar
  :custom
  (pulsar-delay 0.015)
...
(use-package org
  ...
  (org-todo-keyword-faces '(("TODO"  . org-warning      )       ;; org-faces?
                            ("PRJT"  . "tomato"         )
                            ("NEXT"  . "orange"         )
                            ("WAIT"  . "gold"           )
                            ("DONE"  . "dim gray"       )
...
(use-package org-attach
  ...
  (org-attach-id-to-path-function-list '(my/org-attach-dir-calculator
   ...
   (defun my/org-attach-dir (input-fname)
     ""
     (concat "data/"
             ;;(file-name-as-directory (my/org-attach-dir-calculator (org-id-get-create)))
             (file-name-as-directory (my/org-attach-dir-calculator input-fname))
             ;;input-fname
             ))


  ;;(add-to-list org-attach-id-to-path-function-list 'my/org-attach-dir-calculator)
...
** =org-tidy=: hiding the properties
#+begin_src emacs-lisp
...
(use-package denote
  ...
  (defun garid/choose-denote-directory ()
    ""
    (interactive)
    (setq denote-directory
          (completing-read
           ...
           (mapcar
            ...
            (seq-filter
             (lambda (x) (string-match "denote.*" x))         ;; filter only starting with roam
             (directory-files "~/BrainDump/")))   ;; list sub dirs
           ))

    (message "Now denote-directory = %s" denote-directory))
...
(use-package denote-silo
  :custom
  (denote-silo-directories '("~/BrainDump/denote"
...
** test
#+begin_src emacs-lisp
(defun garid/odt-edit-open  (path &optional _) ;;(path)
  (interactive)
  (let (
        (outdir      (file-name-directory      path)) ;; path of the output
        ;;(path_no_ext (file-name-sans-extension path)) ;; filename without extension
        )

    (message (format "lodraw %s && libreoffice --headless --convert-to %s --outdir %s %s" path "svg" outdir path))
    (async-shell-command
     (format "lodraw %s && libreoffice --headless --convert-to %s --outdir %s %s" path "svg" outdir path)
     nil nil
     ;;t t
     )))
...
(use-package prog-mode
  ...
  ) ;; I use M-q for delete-window
#+end_src
** =eldoc=
#+begin_src emacs-lisp
(use-package eldoc-box
  :config
  ;; (evil-define-key 'normal 'prog-mode-map  (kbd "K")
  ;;   (lambda ()
  ;;     (interactive)
...
(use-package code-cells
  ...
  :bind (:map code-cells-mode-map
              ;;("M-[" . code-cells-backward-cell     )
              ;;("M-]" . code-cells-forward-cell      )
              ;;("M-{" . code-cells-move-cell-up      )
              ;;("M-}" . code-cells-move-cell-down    )
...
(use-package jupyter
  ...
  ;; I stopped using the jupter in the org mode after update org-elements
  )
#+end_src
** =graphviz-dot-mode=
#+begin_src emacs-lisp
...
| =nc=     | =garid/choose-denote-directory=        | change denote                | -    |        |
...
(use-package general
  ...
  (general-create-definer garids-keybind/in-prog-mode
    :states '(normal visual)
...
(garids-keybind/for-all-modes
  ...
  "za" '(garid/align-regexp-and-untabify :which-key "align")
  ...
  "nc" '(garid/choose-denote-directory :which-key "change denote")
...
**** manual progmode
#+begin_src emacs-lisp
(garids-keybind/in-prog-mode
  "SPC" '(execute-extended-command      :which-key "")
  ;;  "a" '(python-nav-backward-block       :which-key "block-prev")
  ;;  "e" '(python-nav-forward-block        :which-key "block-next")
  ;;  "[" '(python-nav-backward-defun       :which-key "defun-prev")
  ;;  "]" '(python-nav-forward-defun        :which-key "defun-next")
  ;;  "p" '(python-nav-backward-statement   :which-key "state-prev")
  ;;  "n" '(python-nav-forward-statement    :which-key "state-next")
  ;;  "b" '(python-nav-backward-sexp        :which-key "sexp-prev")
  ;;  "f" '(python-nav-forward-sexp         :which-key "sexp-next")
