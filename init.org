...
(use-package repeat
  :straight nil
  :hook 
  (emacs-startup . repeat-mode) ;; (after-init . repeat-mode)
  )
#+end_src
...
(use-package evil-collection
  ...
  ;; (evil-collection-init) with after-init causes overrides 
...
** =modus= theme                                                                     :by_prot:
[[https://protesilaos.com/emacs/modus-themes][Modus themes]] is a nice built-in theme for emacs that [[https://protesilaos.com/][Protesilaos
Stavrou]]. Following is customization of the theme. Actual in the
loading the theme is in the section [[id:20251007T133457.470613][=emacs=]]
...
(use-package modus-themes
  ...
  :hook
  (after-init . (lambda () (modus-themes-load-theme (car modus-themes-to-toggle))))
  ;; (emacs-startup . (lambda () (modus-themes-load-theme (car modus-themes-to-toggle))))
  )
#+end_src
...
(use-package emacs
  ...
  (emacs-startup . (lambda ()
                     (call-process-shell-command "notify-send \"Emacs\" \"Emacs startup initialized\"")
                     ))
  )
...
(use-package marginalia
  ...
  :hook (after-init . marginalia-mode) ;; :init (marginalia-mode)
  )
#+end_src
...
(use-package org
  ...
  :hook
  (org-after-refile-insert . org-mode-restart)
...
(use-package org-gtd
  ...
  (:states 'normal
           ...
           "o c  "     'org-gtd-capture)
  :hook
  ;; (after-init    . org-gtd-mode)
  (emacs-startup . org-gtd-mode)
  )
#+end_src
...
(use-package tempel
  ...
  :hook
  (conf-mode . (lambda () (setq-local completion-at-point-functions (cons #'tempel-expand completion-at-point-functions))))
  (prog-mode . (lambda () (setq-local completion-at-point-functions (cons #'tempel-expand completion-at-point-functions))))
  (text-mode . (lambda () (setq-local completion-at-point-functions (cons #'tempel-expand completion-at-point-functions))))
  )
...
- [ ] abbrev-mode not working with after-init hook?
...
(use-package abbrev
  ...
  :hook
  (after-init . abbrev-mode) ;; (setq-default abbrev-mode t)
  (org-mode   . abbrev-mode) ;; (setq-default abbrev-mode t)       
  )
#+end_src
...
(use-package diff-mode
  ...
  :bind (
         :map diff-mode-map
         ("M-K" . nil)   ;; was conflict with my window movement
         ("M-J" . nil)   ;; was conflict with my window movement
         )
  )
#+end_src
...
(use-package eshell
  ...
  (emacs-startup . eshell-atuin-mode)
  )
#+end_src
...
(use-package gptel
  ...
  ;; (gptel-model        'gpt-5.2)
  ;; (gptel-backend      (gptel-make-openai "OpenAI" :key 'gptel-api-key-from-auth-source))
...
(use-package gptel-commit
  ...
  ;; :config 
  ;; (define-key evil-normal-state-map (kbd "SPC g c") 'gptel-commit          )  ;; needed to use this
  ;; (define-key evil-normal-state-map (kbd "SPC g C") 'gptel-commit-rationale)  ;; because :bind/:general won't 
  )
#+end_src
...
;; :hook (org-mode . jinx-mode)) sometimes too intrusive 
...
(use-package captain
  ...
  :hook
  (after-init . global-captain-mode) ;; :config (global-captain-mode t)
